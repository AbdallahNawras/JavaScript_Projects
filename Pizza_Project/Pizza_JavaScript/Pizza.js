...
function getVegetables(runningTotal,text1) {
    ...
    for(var i = 0; i < veggieArray.length; i++) {
        if(veggieArray[i].checked) {
            ...
            console.log("selected vegie item: (" + veggieArray[i].value+ ")");
            text1 = text1+veggieArray[i].value+"<br>";
        }
    }
    var veggieCount = selectedVeggie.length;
    console.log("total vigge are: " + veggieCount);
...
function getMeat(runningTotal,text1) {
    ...
    for (var j = 0; j < meatArray.length; j++) {
        if(meatArray[j].checked) {
            selectedMeat.push(meatArray[j].value);//push will add element at the end of the array.
